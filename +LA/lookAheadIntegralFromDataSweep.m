...
for k = 1:length(kappaSegments) - 1
    ...
    for i = 1:nLookAheadDistances
        ...
        for j = 1:size(segmentData,1)
            ...
            kappaIntegrate = Utilities.fnGetLookAheadValues(kappaInterp, closestWaypoints(j), 1/0.1, lookAheadDistances(i));

            % Integrate curvature
            distVector = (1:lookAheadDistances(i));
            lookAheadKappa(j) = trapz(distVector, kappaIntegrate);
